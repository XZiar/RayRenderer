...
#define CASE(str, dtype, bit, n, least) \
    HashCase(type, str) return { common::simd::VecDataInfo::DataTypes::dtype, n, 0, bit, least ? VTypeInfo::TypeFlags::MinBits : VTypeInfo::TypeFlags::Empty };
...
#define CASE2(tstr, type, bit)                  \
    ...
    return { VecDataInfo::DataTypes::Empty, 0, 0, 0, VTypeInfo::TypeFlags::Empty };
...
{
#define CASE(s, type, flag, bit, n) \
    case static_cast<uint32_t>(VTypeInfo{ VecDataInfo::DataTypes::type, n, 0, bit, VTypeInfo::TypeFlags::flag }): return U"" STRINGIZE(s) ""sv;
#define CASE2(s, type, bit, n)                  \
    CASE(s,           type, Empty,   bit, n)    \
    CASE(PPCAT(s, +), type, MinBits, bit, n)
#define CASEV(pfx, type, bit) \
    CASE2(pfx,             type, bit, 1)  \
    CASE2(PPCAT(pfx, v2),  type, bit, 2)  \
    CASE2(PPCAT(pfx, v3),  type, bit, 3)  \
    CASE2(PPCAT(pfx, v4),  type, bit, 4)  \
    CASE2(PPCAT(pfx, v8),  type, bit, 8)  \
    CASE2(PPCAT(pfx, v16), type, bit, 16) \
