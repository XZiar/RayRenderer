...
{
    auto pms = Executor.AddTask([=](const common::asyexe::AsyncAgent& agent)
    {
        const auto vao = flipY ? FlipYVAO : NormalVAO;
        const auto outformat = TexFormatUtil::ConvertFrom(output, true);
        tex->CheckCurrent();
    ...
    return ExtractImage(std::move(pms), output);
}
template<>
