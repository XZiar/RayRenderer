...
#define CvtF32I32Args BOOST_PP_VARIADIC_TO_SEQ(dest, src, count, mulVal)
#define CvtF32I16Args BOOST_PP_VARIADIC_TO_SEQ(dest, src, count, mulVal)
#define CvtF32I8Args  BOOST_PP_VARIADIC_TO_SEQ(dest, src, count, mulVal)
...
template<template<typename, typename> class Cast, typename Src, typename Dst, typename... Args>
static void CastLoop(Dst* dest, const Src* src, size_t count, Args&&... args) noexcept
{
    Cast<Src, Dst> cast(std::forward<Args>(args)...);
...
DEFINE_FASTPATH_METHOD(CvtF32I32, LOOP)
{
    if (mulVal == 0)
        CastLoop<ScalarCast>(dest, src, count);
    else
...
DEFINE_FASTPATH_METHOD(CvtF32I16, LOOP)
{
    if (mulVal == 0)
        CastLoop<ScalarCast>(dest, src, count);
    else
...
DEFINE_FASTPATH_METHOD(CvtF32I8, LOOP)
{
    if (mulVal == 0)
        CastLoop<ScalarCast>(dest, src, count);
    else
...
template<typename TIn, typename TCast, typename T>
struct LoadCastSave
{
    ...
    void Proc(Dst* dst, const Src* src, std::index_sequence<I...>) const noexcept
    {
        ...
        if constexpr (N == M)
        {
            ...
            self.Post(val.template Cast<TCast>()).Save(dst);
        }
        else if constexpr (N < M)
        {
            const TIn val[] = { self.Pre(TIn(src)), self.Pre(TIn(src + N + N * I))... };
            self.Post(val[0].template Cast<TCast>(val[1 + I]...)).Save(dst);
        }
        else
        {
            const auto mid = self.Pre(TIn(src)).template Cast<TCast>();
            (..., self.Post(mid[I]).Save(dst + M * I));
        }
    }
};
template<typename TIn, typename TCast>
struct DefaultCast : public LoadCastSave<TIn, TCast, DefaultCast<TIn, TCast>>
{
...
template<typename TIn, typename TCast>
struct I2FCast : public LoadCastSave<TIn, TCast, I2FCast<TIn, TCast>>
{
    TCast Muler;
    I2FCast(TCast mulVal) noexcept : Muler(mulVal) {}
    TIn Pre(TIn val) const noexcept { return val; }
    TCast Post(TCast val) const noexcept { return val.Mul(Muler); }
};
template<typename TIn, typename TCast>
struct F2ICast : public LoadCastSave<TIn, TCast, F2ICast<TIn, TCast>>
{
    TIn Muler;
    F2ICast(TIn mulVal) noexcept : Muler(mulVal) {}
...
DEFINE_FASTPATH_METHOD(CvtF32I32, SIMD128)
{
    if (mulVal == 0)
        CastSIMD4<DefaultCast<simd::F32x4, simd::I32x4>, &GET_FASTPATH_METHOD(CvtF32I32, LOOP)>(dest, src, count, mulVal);
    else
        CastSIMD4<F2ICast<simd::F32x4, simd::I32x4>, &GET_FASTPATH_METHOD(CvtF32I32, LOOP)>(dest, src, count, mulVal);
}
...
{
    CastSIMD4<DefaultCast<simd::U16x8, simd::U8x16>, &GET_FASTPATH_METHOD(NarrowCopy21, LOOP)>(dest, src, count);
}
DEFINE_FASTPATH_METHOD(NarrowCopy41, SIMD128)
{
    CastSIMD4<DefaultCast<simd::U32x4, simd::U8x16>, &GET_FASTPATH_METHOD(NarrowCopy41, LOOP)>(dest, src, count);
}
DEFINE_FASTPATH_METHOD(NarrowCopy42, SIMD128)
{
    CastSIMD4<DefaultCast<simd::U32x4, simd::U16x8>, &GET_FASTPATH_METHOD(NarrowCopy42, LOOP)>(dest, src, count);
}
#endif
...
{
    CastSIMD4<DefaultCast<simd::U16x16, simd::U8x32>, &GET_FASTPATH_METHOD(NarrowCopy21, SIMDSSSE3)>(dest, src, count);
}
DEFINE_FASTPATH_METHOD(NarrowCopy41, SIMDAVX2)
{
    CastSIMD4<DefaultCast<simd::U32x8, simd::U8x32>, &GET_FASTPATH_METHOD(NarrowCopy41, SIMDSSSE3)>(dest, src, count);
}
DEFINE_FASTPATH_METHOD(NarrowCopy42, SIMDAVX2)
{
    CastSIMD4<DefaultCast<simd::U32x8, simd::U16x16>, &GET_FASTPATH_METHOD(NarrowCopy42, SIMDSSSE3)>(dest, src, count);
}
