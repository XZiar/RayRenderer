...
std::vector<uint8_t> ResourceHelper::getData(const wchar_t *type, const int32_t id)
{
	...
	if (NULL == hRsrc)
		COMMON_THROW(BaseException, L"Failed to find resource");
	DWORD dwSize = SizeofResource(hdll, hRsrc);
	if (0 == dwSize)
		COMMON_THROW(BaseException, L"Resource has zero size");
	HGLOBAL hGlobal = LoadResource(hdll, hRsrc);
	if (NULL == hGlobal)
		COMMON_THROW(BaseException, L"Failed to load resource");
	LPVOID pBuffer = LockResource(hGlobal);
	if (NULL == pBuffer)
		COMMON_THROW(BaseException, L"Failed to lock resource");
