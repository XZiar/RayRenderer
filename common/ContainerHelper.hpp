...
struct StdContiguous : DoContiguous
{
    ...
    using EleType = common::remove_cvref_t<decltype(*Data(std::declval<const T&>()))>;
    constexpr static size_t EleSize = sizeof(EleType);
};
}
template<typename T>
...
struct ContiguousHelper<std::basic_string_view<Ch>> : detail::StdContiguous<std::basic_string_view<Ch>> {};
template<>
