...
forceinline bool ShortCompare(const std::basic_string_view<T> src, const T(&target)[N]) noexcept
{
    ...
    else if constexpr (targetSBytes <= 128u / 8u)
    {
        ...
        const auto cmpbits = static_cast<uint32_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS, vecT)));
    ...
    else if constexpr (targetSBytes <= 256u / 8u)
    {
        ...
        const auto cmpbits0 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS0, vecT0)));
        const auto cmpbits1 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS1, vecT1)));
        return cmpbits0 == UINT16_MAX && (cmpbits1 & mask) == mask;
#else
        return src == target;
#endif
    }
    else if constexpr (targetSBytes <= 384u / 8u)
    {
        ...
        const auto vecS0 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(src.data()) + 0);
        const auto vecS1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 2);
        ...
        const auto vecT0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 0);
        const auto vecT1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 1);
        const auto vecT2 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 2);
        const auto vecS0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 0);
        const auto vecS1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 1);
        const auto vecS2 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 2);
        const auto cmpbits0 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS0, vecT0)));
        const auto cmpbits1 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS1, vecT1)));
        const auto cmpbits2 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS2, vecT3)));
        return cmpbits0 == UINT16_MAX && cmpbits1 == UINT16_MAX && (cmpbits2 & mask) == mask;
#else
        return src == target;
#endif
    }
    else if constexpr (targetSBytes <= 512u / 8u)
    {
        ...
        const auto vecS0 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(src.data()) + 0);
        const auto vecS1 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(src.data()) + 1);
        const auto cmpbits0 = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(vecS0, vecT0)));
        ...
        const auto vecT1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 1);
        const auto vecT2 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 2);
        const auto vecT3 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(target) + 3);
        const auto vecS0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 0);
        const auto vecS1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 1);
        const auto vecS2 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 2);
        const auto vecS3 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src.data()) + 3);
        const auto cmpbits0 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS0, vecT0)));
        const auto cmpbits1 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS1, vecT1)));
        const auto cmpbits2 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS2, vecT2)));
        const auto cmpbits3 = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(vecS3, vecT3)));
        return cmpbits0 == UINT16_MAX && cmpbits1 == UINT16_MAX && cmpbits2 == UINT16_MAX && (cmpbits3 & mask) == mask;
#else
        return src == target;
#endif
    }
