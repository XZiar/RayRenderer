...
BasicTest::BasicTest()
{
    try
    {
        ...
        FontView = gcnew Common::Controllable(core->GetFontViewer());
        GLShaders = gcnew List<Common::Controllable^>(0);
        for (const auto& glshader : core->GLShaders())
            GLShaders->Add(gcnew Common::Controllable(glshader));
    }
...
Common::Controllable^ BasicTest::AddShader(CLIWrapper<Wrapper<rayr::GLShader>>^ theShader)
{
    auto shader = theShader->Extract();
    auto control = gcnew Common::Controllable(shader);
    GLShaders->Add(control);
    return control;
}
Task<Common::Controllable^>^ BasicTest::AddShaderAsync(String^ fname, String^ shaderName)
{
    return doAsync3<Common::Controllable^>(gcnew Func<CLIWrapper<Wrapper<rayr::GLShader>>^, Common::Controllable^>(this, &BasicTest::AddShader),
        &rayr::BasicTest::LoadShaderAsync, *core, ToU16Str(fname), ToU16Str(shaderName));
}
