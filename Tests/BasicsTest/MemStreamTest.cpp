...
{
    {
        ...
        EXPECT_EQ(memStream.GetSize(), 3u);
...
{
    {
        ...
        MemoryInputStream memStream(common::to_span(dat));
        EXPECT_EQ(memStream.GetSize(), 10u);
        EXPECT_EQ(memStream.CurrentPos(), 0u);

        EXPECT_EQ(memStream.ReadByte(), std::byte{ 0 });
        EXPECT_EQ(memStream.CurrentPos(), 1u);

        EXPECT_TRUE(memStream.SetPos(9));
        EXPECT_EQ(memStream.CurrentPos(), 9u);
        EXPECT_EQ(memStream.ReadByte(), std::byte{ 9 });
        EXPECT_EQ(memStream.CurrentPos(), 10u);
        EXPECT_TRUE(memStream.IsEnd());

        EXPECT_TRUE(memStream.SetPos(5));
        EXPECT_EQ(memStream.CurrentPos(), 5u);
        EXPECT_EQ(memStream.ReadByte(), std::byte{ 5 });
        EXPECT_EQ(memStream.CurrentPos(), 6u);
...
TEST(MemStream, ReadInto)
{
    ...
    MemoryInputStream memStream(common::to_span(dat));
    EXPECT_EQ(memStream.GetSize(), 6u);
    ...
    EXPECT_EQ(memStream.CurrentPos(), 4u);
    EXPECT_EQ(memStream.ReadTo(ret, 10), 2u);
    ...
    EXPECT_TRUE(memStream.SetPos(4));
    EXPECT_EQ(memStream.CurrentPos(), 4u);
    EXPECT_EQ(memStream.ReadInto(ret2, 1), 2u);
...
{
    {
        ...
        EXPECT_EQ(contStream.GetSize(), 10u);
        EXPECT_EQ(contStream.CurrentPos(), 0u);

        EXPECT_TRUE(contStream.Skip(5));
        EXPECT_EQ(contStream.CurrentPos(), 5u);
        EXPECT_EQ(contStream.ReadByte(), std::byte{ 5 });
        EXPECT_EQ(contStream.CurrentPos(), 6u);
    }
    {
...
TEST(MemStream, Write)
{
    ...
    }
    {
        ...
        std::vector<uint32_t> dat(src.size(), 0u);
...
TEST(MemStream, ContainerWrite)
{
        ...
        src.push_back(n);
    {
        ...
        EXPECT_EQ(contStream.GetSize(), 0u);
        ...
        EXPECT_EQ(contStream.WriteFrom(src, 11, 3), 3u);
        ...
        EXPECT_EQ(contStream.WriteFrom(src, 12, 10), 16u - 12u);
