...
// ++UTF++
typedef basic_parse_context<char16_t> u16format_parse_context;
typedef basic_parse_context<char32_t> u32format_parse_context;
...
struct MakeValueProxy
{
	...
	{
		return make_value<C>(val);
	}
};

template <typename Context, typename T>
struct get_type {
  typedef decltype(MakeValueProxy<Context>::make(
...
FMT_CONSTEXPR basic_format_arg<Context> make_arg(const T &value) {
  ...
  arg.value_ = MakeValueProxy<Context>::make(value);
  return arg;
}
...
inline typename std::enable_if<IS_PACKED, value<Context>>::type
  ...
  return MakeValueProxy<Context>::make(value);
}
