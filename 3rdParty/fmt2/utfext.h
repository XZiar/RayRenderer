    ...
    iterator operator()(basic_string_view<char_type> value)
    {
        ...
        case 2:
            if constexpr (sizeof(char_type) == 2)
                ...
                return internal::arg_formatter_base<Range>::operator()(
                    CharsetConvertor<UTF16, UTF32, const char16_t, char_type>::Convert(reinterpret_cast<const char16_t*>(value.data()), realSize, true, true));
        case 4:
            if constexpr (sizeof(char_type) == 4)
                return internal::arg_formatter_base<Range>::operator()(basic_string_view<char_type>(value.data(), realSize));
            else // UTF16
                return internal::arg_formatter_base<Range>::operator()(
                    CharsetConvertor<UTF32, UTF16, const char32_t, char_type>::Convert(reinterpret_cast<const char32_t*>(value.data()), realSize, true, true));
        }
        return this->out();
    }
};
