...
struct SubroutineItem
{
        ...
        std::map<std::string, ShaderExtInfo::EmSubroutine>& emulateOutput) const
    {
        ...
        else
        {
            ...
            {
                fmter.FormatToStatic(entry, FmtString("{} {}({});\r\n"sv), ReturnType, routine, FuncParams);
            }
            entry.append("{2} {1}({3})\r\n");
            entry.append("{{\r\n");
            ...
            {
                fmter.FormatToStatic(entry, FmtString("    case {}u: return {}({});\r\n"sv), srline, srname, args);
        ...
        {
            lines[LineNum] = fmter.FormatDynamic(entry, SubroutineName, RoutineVal, ReturnType, FuncParams, Routine);
        }
        for (const auto&[routine, rtline] : Routines)
        {
            lines[rtline] = fmter.FormatStatic(FmtString("{0}{1} {2}({3})"sv), prefix, ReturnType, routine, FuncParams);
        }
    }
...
vector<oglShader> oglShader_::LoadFromExSrc(const string& src, ShaderExtInfo& info, const ShaderConfig& config, const bool allowCompute, const bool allowDraw)
{
    ...
    for (const auto& stype : stypes)
    {
        ...
        finalShader
            .append(partHead)
            .append(common::str::Formatter<char>{}.FormatStatic(FmtString("{}{}{}\r\n"sv), verPrefix, curVer, verSuffix))
