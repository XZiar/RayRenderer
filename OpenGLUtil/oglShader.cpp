...
vector<oglShader> oglShader::LoadFromExSrc(const string& src, ShaderExtInfo& info, const ShaderConfig& config, const bool allowCompute, const bool allowDraw)
{
    ...
    for (const auto& stype : stypes)
    {
        ...
        oglShader shader(shaderType, finalShader);
        shaders.push_back(shader);
    }
    return shaders;
}
