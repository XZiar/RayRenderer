...
constexpr std::pair<uint32_t, std::string_view> MSG_NAMES[] = {
    {0,     "WM_NULL" },
    {1,     "WM_CREATE" },
    {2,     "WM_DESTROY" },
    {3,     "WM_MOVE" },
...
static std::string_view GetMessageName(const uint32_t msg) noexcept
{
    using T = std::pair<uint32_t, std::string_view>;
    using Comp = common::container::SetKeyLess<T, &T::first>;
    static common::container::FrozenDenseSet<T, Comp> MsgMap(std::vector(std::cbegin(MSG_NAMES), std::cend(MSG_NAMES)));
    const auto item = MsgMap.Find(msg);
    return item == nullptr ? "UNKNOWN" : item->second;
}
