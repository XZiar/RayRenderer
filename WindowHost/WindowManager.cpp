...
void FillBufferColorXXXA(uint32_t* ptr, uint32_t width, uint32_t height, uint32_t rowStride, uint32_t idx) noexcept
{
    for (uint32_t h = 0; h < height; ++h)
    {
        const auto hval = h * 255u / height;
        const auto rowOffset = h * rowStride;
        for (uint32_t w = 0; w < width; ++w)
        {
            const auto wval = w * 255u / width;
            const auto clr = ((hval << 8) + wval) << ((idx & 0x1u) ? 8 : 0);
            ptr[rowOffset + w] = 0xff000000u + clr;
        }
    }
}
...
public:
    ...
    static inline const auto Dummy = []()
    {
        try
        {
            xziar::gui::detail::RegisterInteraction(std::make_unique<XDGInteraction>());
            return true;
        }
